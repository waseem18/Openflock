		...
		if self.session.get('username'):
			...
			if split_result.netloc=="github.com" and len(split_result.path.split('/'))>=3 and split_result.path.split('/')[1]==str(username_session) and split_result.path.split('/')[2]:
				...
				if is_repo_present>0 and len(split_result.path.split('/'))==3:
					self.redirect('http://www.openflock.co/promote/'+str(promoting_repo)+'?project=user')
					...
					self.redirect('http://www.openflock.co/promote/'+str(promoting_repo)+'?issue=True&id='+str(issue_id)+'&url_entered='+str(url_entered)+'&project=user')
				else:
				...
				if cire.status_code == 200 and len(split_result.path.split('/'))==3:
					self.redirect('http://www.openflock.co/promote'+'/'+str(split_result.path.split('/')[2])+'?project=org&organization='+str(split_result.path.split('/')[1]))
					...
					issue_id = split_result.path.split('/')[4]
					self.redirect('http://www.openflock.co/promote/'+str(split_result.path.split('/')[2])+'?issue=True&id='+str(issue_id)+'&url_entered='+str(url_entered)+'&project=org&organization='+str(split_result.path.split('/')[1]))
				else:
					self.response.out.write("<h3>There's something wrong with the url. Check the URL once clearly.</h3>")
